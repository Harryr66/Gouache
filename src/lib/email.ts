...
import { RefundRequestEmail as RefundRequestTemplate } from '@/emails/refund-request';
...
export interface RefundRequestEmail {
  sellerEmail: string;
  sellerName: string;
  buyerName: string;
  buyerEmail: string;
...
export async function sendRefundRequestEmail(data: RefundRequestEmail) {
  const resendApiKey = process.env.RESEND_API_KEY;
  const fromAddress = process.env.RESEND_FROM_EMAIL || 'Gouache <hello@gouache.art>';
  ...
  try {
    const result = await resend.emails.send({
      from: fromAddress,
      to: data.sellerEmail,
      subject: `Refund Request for ${data.itemTitle}`,
      html: emailHtml,
    });

    console.log('✅ Refund request email sent to seller');
    return result;
  } catch (error) {
