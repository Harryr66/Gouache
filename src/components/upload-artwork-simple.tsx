...
/**
 * SIMPLE Artwork Upload - Rebuilt to avoid React error #300
 * Uses proper event handlers and avoids state updates in callbacks
 */
export function UploadArtworkSimple() {
  const { user } = useAuth();
  ...
  // Handle file selection - use useCallback to prevent re-renders
  const handleFileChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    const selectedFiles = Array.from(e.target.files || []);
    if (selectedFiles.length === 0) return;

    // Update files state immediately (this is safe in event handler)
      ...
      reader.onload = (event) => {
        ...
        if (result) {
          ...
          if (loadedCount === selectedFiles.length) {
            // Use requestAnimationFrame to ensure we're not in render
  ...
  // Submit handler - all async work here
  ...
  }, [user, files, title, description, isForSale, isOriginal, priceType, price, deliveryScope, addContent, router]);
